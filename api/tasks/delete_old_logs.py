...
def delete_old_logs(_self: Task | None = None) -> None:
    ...
    logger.info("Starting task: delete_old_logs. Deleting files older than %s.", cutoff_date.date())

    if not os.path.isdir(LOGS_DIR):
        logger.warning("Logs directory not found at: %s. Skipping task.", LOGS_DIR)
        return
    ...
    for log_file in os.listdir(LOGS_DIR):
        ...
        if log_file.startswith("fastapi.log."):
            ...
            try:
                ...
                logger.warning("Could not parse date from log file: %s", log_file)
                continue
    logger.info("Finished task: delete_old_logs.")
