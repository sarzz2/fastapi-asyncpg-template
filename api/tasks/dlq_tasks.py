...
def clean_old_dlq_tasks(self: Task, days: int = DLQConstants.DEFAULT_RETENTION_DAYS) -> None:
    ...
    """
    log.info("Starting clean_old_dlq_tasks: purging entries older than %d days.", days)
    ...
    log.info("Finished clean_old_dlq_tasks: purged %d old DLQ entries.", deleted_count)
